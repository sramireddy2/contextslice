...
- Call Figma the minimum number of times: verify the token on a cheap endpoint, do **one** full
  file fetch streamed to disk, and (later, if needed) **one** batched image export.
  No retry loops: on HTTP 429, print the headers and stop.
